...
import { Trash2Icon, WalletIcon, ShieldCheck, ShieldAlert, PlusIcon } from "lucide-react";
    ...
    <div className="text-left">
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogTrigger asChild>
            <Button variant="secondary" className="w-full" iconLeft={<PlusIcon />}>Add Wallet</Button>
        ...
        <DialogContent>
          ...
          <DialogFooter>
            <Button onClick={handleAddWallet} disabled={!form.currency || !form.network || !form.address}>
              Add
            </Button>
            <DialogClose asChild>
      ...
      <div className="flex flex-col gap-3 mt-6">
        {wallets.length === 0 ? (
          <div className="flex flex-col items-center justify-center bg-[#18181b] rounded-lg px-4 py-8 text-center">
            ...
            <Typography variant="label/sm" className="text-muted-foreground mt-1">
              Add your first wallet to start accepting crypto payments
            </Typography>
          </div>
        ) : (
          wallets.map((wallet, idx) => {
            const currencyOption = CURRENCY_OPTIONS.find((c) => c.value === wallet.currency);
            const CurrencyIcon = getCurrencyIcon(wallet.currency);
            return (
              <div key={idx} className="flex items-center justify-between bg-[#18181b] rounded-lg px-4 py-3">
                <div className="flex items-center gap-3">
                  <CurrencyIcon className="w-6 h-6" />
                  <Typography variant="label/md" className="font-bold">
                    {currencyOption?.label} ({NETWORK_OPTIONS.find((n) => n.value === wallet.network)?.label})
                  </Typography>
                ...
                <div className="flex items-center gap-2">
                  <Chip
                    variant={wallet.status === "confirmed" ? "success" : "secondary"}
                    size="sm"
                  >
                    {wallet.status === "confirmed" ? "Confirm" : "Pend Confirm"}
                  </Chip>
                  <Button variant="secondary" size="icon" onClick={() => handleDelete(idx)}>
