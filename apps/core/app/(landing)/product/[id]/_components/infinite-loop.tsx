    ...
    <div className="flex flex-col relative pt-7">
      <AnimatePresence mode="wait">
        {items.map((item, index) => {
          ...
          const isAbove = index === (currentIndex - 1 + items.length) % items.length;
          const isBelow = index === (currentIndex + 1) % items.length;
          const isAbove2 = index === (currentIndex - 2 + items.length) % items.length;
          const isBelow2 = index === (currentIndex + 2) % items.length;
          
          if (!isMain && !isAbove && !isBelow && !isAbove2 && !isBelow2) return null;
          ...
          return (
            <motion.div
              ...
              initial={{ 
                y: isAbove ? -60 : isBelow ? 60 : isAbove2 ? -120 : isBelow2 ? 120 : 0,
            ...
            >
              ...
              <p className={`font-bold whitespace-nowrap ${isMain ? 'text-4xl' : 'text-3xl'}`}>
                {item.name}
              </p>
            </motion.div>
          );
