...
export const MagicCursor: React.FC = () => {
  ...
  const updatePosition = (isScrolling = false) => {
    ...
    // Animate to target - consolidated animations with the same options
    const movementOptions = {
      ...ANIMATION_SETTINGS.MOVEMENT,
      onComplete: () => {
        // After reaching target position, trigger click animation
        if (taskType === "click") {
          performClick({
            target,
            callback: () => {
    ...
    animate(y, targetY, ANIMATION_SETTINGS.MOVEMENT);
